...
const Drawdown = () => {  
  ...
  const compoundInterest = Math.pow(1 + interest, parseFloat(time.value));
  const principalInterest = amount * compoundInterest;

  const drawRate = Math.pow(1 + interest, parseFloat(time.value) + 1) - 1;
  ...
  return (
    <div style={style}>
      <input type="number" step="1000" value={amount} onChange={(e) => setAmount(e.currentTarget.value) } />
      ...
      <div style={{marginTop: '2rem'}}>
        ...
        <strong>Draw Interest</strong>: {drawRate}<br />
        <strong>Draw Amount</strong>: {format(drawAmount)}<br />
        <strong>Draw Total</strong>: {format(drawTotal)}<br />
